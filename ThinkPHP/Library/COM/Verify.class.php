...
class Verify {
	 ...
	 *  @param   integer   $imageH    图片宽
	 *  @param   integer   $imageL    图片长
	 *  @param   integer   $length    验证码位数
	 *  @param   string    $fontttf   验证码字体，不设置随机获取
	 */
	public function setImage ($fontSize, $imageH, $imageL, $length, $fontttf) {
		$this->fontSize = !empty($fontSize) ? $fontSize : 25;
		$this->imageH   = !empty($imageH) ? $imageH : 0;
		$this->imageL   = !empty($imageL) ? $imageL : 0;
		$this->length   = !empty($length) ? $length : 4;
		$this->fontttf  = !empty($fontttf) ? $fontttf : '';
	}
	...
	public function entry($id = '') {
		// 图片宽(px)
		$this->imageL || $this->imageL = $this->length*$this->fontSize*1.5 + $this->length*$this->fontSize/4;
		// 图片高(px)
		$this->imageH || $this->imageH = $this->fontSize * 2.3;
